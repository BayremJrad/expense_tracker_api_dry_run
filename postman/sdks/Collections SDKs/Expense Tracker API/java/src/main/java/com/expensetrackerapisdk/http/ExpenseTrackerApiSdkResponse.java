 ...
 */
@Getter
public class ExpenseTrackerApiSdkResponse<T> {
  ...
  private final ExpenseTrackerApiSdkResponseMetadata metadata;
   ...
   */
  public ExpenseTrackerApiSdkResponse(Response response, byte[] bodyBytes, T data) {
    this.data = data;
    this.raw = new RawHttpResponse(bodyBytes, response);
    this.metadata = new ExpenseTrackerApiSdkResponseMetadata(
      response.code(),
